...
const ChatList: React.FC<ChatListProps> = ({ onSelectChat, selectedChatId }) => {
  const { chats, deleteChat, archiveChat } = useMessaging();
  ...
  const handleDeleteChat = useCallback((e: React.MouseEvent, chat: ChatItem) => {
    e.stopPropagation(); // Prevent selecting the chat when clicking delete
    
    // Debug permissions
    console.log(`Chat permissions for ${chat.id} (${chat.display_name}):`, {
      can_delete: chat.can_delete,
      can_archive: chat.can_archive,
      created_by: chat.created_by,
      created_by_username: chat.created_by_username
    });
    
    // Check if user has permission to delete or archive/hide
    if (chat.can_delete) {
      console.log(`Attempting to delete chat ${chat.id} (user has delete permission)`);
      if (window.confirm('Are you sure you want to permanently delete this chat for all members?')) {
        deleteChat(chat.id).catch(error => {
          console.error('Failed to delete chat:', error);
          alert('Failed to delete chat. Please try again.');
        });
      }
    } else if (chat.can_archive) {
      console.log(`Attempting to archive chat ${chat.id} (user does not have delete permission)`);
      if (window.confirm('Are you sure you want to hide this chat? It will be removed from your view but will still be visible to other chat members.')) {
        archiveChat(chat.id).catch(error => {
              ...
              <div className="px-4 py-3">
                <div className="flex justify-between">
                  ...
                  <div className="flex items-center">
                    ...
                    {(chat.can_delete || chat.can_archive) && (
                      <button 
                        onClick={(e) => handleDeleteChat(e, chat)}
                        className="flex items-center text-gray-500 hover:text-gray-700 transition-colors text-xs"
                        title={chat.can_delete ? "Delete chat for everyone" : "Hide chat (only for you)"}
                      >
                        {chat.can_delete ? (
                          <>
                            <span className="mr-1 text-red-500">Delete</span>
